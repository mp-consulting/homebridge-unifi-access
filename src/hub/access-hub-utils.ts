...
// Return the door service type from configuration. UA Gate devices default to GarageDoorOpener and can be overridden to Lock. Other hubs default
// to Lock and can be overridden to GarageDoorOpener.
...
export function isWired(hub: AccessHub, input: SensorInput): boolean {
  ...
  if(sensorConfig.proxyMode) {
    ...
    return hub.uda.extensions?.[0]?.target_config
      ?.some(e => (e.config_key === REX_BUTTON_MODE_CONFIG_KEY) && (e.config_value === sensorConfig.proxyMode)) ?? false;
  }
