...
export const renderOptions = () => {
  ...
  state.categories.forEach((category) => {
    ...
    if(scope.device && (scope.device.display_model !== 'controller')) {
      ...
      if(!catModelKey.some(m => [ 'all', scope.device.display_model ].includes(m))) {
        return; 
      }

      if(catCapability && (!scope.device.capabilities || !catCapability.some(c => scope.device.capabilities.includes(c)))) {
        return; 
      }
    }
    ...
    let validOptions = categoryOptions.filter(opt => {
      ...
      if(scope.device && (scope.device.display_model !== 'controller')) {
        ...
        if(opt.hasCapability && (!scope.device.capabilities || !opt.hasCapability.some(c => scope.device.capabilities.includes(c)))) {
          return false; 
        }

        if(opt.modelKey && (opt.modelKey !== 'all') && !opt.modelKey.includes(scope.device.display_model)) {
          return false; 
        }
      }
...
const createOptionItem = (optionKey, opt, scope, category) => {
  ...
  if(opt.defaultValue !== undefined) {
    ...
    const unitMatch = opt.description.match(/\bin (seconds?|minutes?|hours?)\b/i);
